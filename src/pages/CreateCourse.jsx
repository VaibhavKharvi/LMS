...
import { PlusCircle, FileText, Image as ImageIcon, DollarSign, List, Trash2 } from 'lucide-react';
    ...
    return (
        <div style={styles.container}>
            <div style={styles.header}>
                ...
                <p style={styles.subtitle}>Fill in the details to launch your new learning experience.</p>
            </div>
                ...
                <div className="glass-card" style={styles.card}>
                    ...
                    <div style={styles.inputGrid}>
                        <div style={styles.inputGroup}>
                            ...
                            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Master React" required style={styles.input} />
                        </div>
                        <div style={styles.inputGroup}>
                            <label style={styles.label}>Category</label>
                            <input type="text" value={category} onChange={(e) => setCategory(e.target.value)} placeholder="e.g. Development" required style={styles.input} />
                        </div>
                        <div style={styles.inputGroup}>
                            <label style={styles.label}>Price (₹)</label>
                            <input type="number" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="0.00" required style={styles.input} />
                        </div>
                        <div style={styles.inputGroup}>
                            <label style={styles.label}>Thumbnail URL</label>
                            <input type="text" value={thumbnail} onChange={(e) => setThumbnail(e.target.value)} placeholder="https://..." style={styles.input} />
                        </div>
                    </div>
                    <div style={styles.inputGroup}>
                        <label style={styles.label}>Description</label>
                        <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Tell students about this course..." required style={{ ...styles.input, height: '100px', resize: 'none' }} />
                    </div>
                </div>
                    ...
                    {lessons.map((lesson, index) => (
                        <div key={index} style={styles.lessonItem}>
                            ...
                            <div style={styles.inputGrid}>
                                ...
                                <input type="text" value={lesson.videoUrl} onChange={(e) => handleLessonChange(index, 'videoUrl', e.target.value)} placeholder="Video URL (Optional)" style={styles.input} />
                            </div>
                            <textarea value={lesson.content} onChange={(e) => handleLessonChange(index, 'content', e.target.value)} placeholder="Lesson Content/Description" required style={{ ...styles.input, height: '80px', marginTop: '0.75rem', resize: 'none' }} />
                        </div>
                    ))}
                </div>
...
const styles = {
    ...
    addBtn: { background: 'transparent', color: 'var(--primary)', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: '500' },
    removeBtn: { background: 'transparent', color: '#ef4444' },
    submitBtn: { width: '100%', justifyContent: 'center', height: '50px', fontSize: '1.1rem' },
};
